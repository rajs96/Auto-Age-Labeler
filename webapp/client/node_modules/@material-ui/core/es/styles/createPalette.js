...
export default function createPalette(palette) {
  ...
  function getContrastText(background) {
    if (process.env.NODE_ENV !== 'production') {
      if (!background) {
        console.error(`Material-UI: missing background argument in getContrastText(${background}).`);
      }
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (process.env.NODE_ENV !== 'production') {
        if (contrast < 3) {
          console.error([`Material-UI: the contrast ratio of ${contrast}:1 for ${contrastText} on ${background}`, 'falls below the WACG recommended absolute minimum contrast ratio of 3:1.', 'https://www.w3.org/TR/2008/REC-WCAG20-20081211/#visual-audio-contrast-contrast'].join('\n'));
        }
      }
    }
